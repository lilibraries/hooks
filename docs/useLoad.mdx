...
- **`options`:** Options object.
  ...
  - **`options.imperative`:** Run callback manually through the returned `load` or `force` function, rather than automatically after the component is mounted.
  ...
  - **`options.cacheKey`:** Cache key. It can be any type except `null` and `undefined`.
